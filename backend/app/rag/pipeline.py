...
10. Generation (Grounded LLM synthesis with anti-hallucination guardrails)
"""
...
class RAGPipeline:
    ...
    async def generate_grounded_answer(
        ...
        """Step 1-10: Execute end-to-end RAG pipeline from retrieval through synthesis and citation mapping."""
        start_time = time.time()

        # Retrieve and select minimal sufficient context
        ...
        if not context_res.selected_chunks:
            # Fallback if no relevant passages are discovered for tenant
            answer = (
                f"No verified documents found for tenant `{tenant_id}`. "
        ...
        if not raw_answer:
            # Deterministic synthesis fallback from top selected evidence
        ...
        # Guarantee citations have tenant boundary validated
