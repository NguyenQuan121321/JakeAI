"""DeepSeek LLM Provider Adapter with 64-Token Automatic Context Caching.

Owns:
1. Credential injection (BYOK tenant key with platform key fallback).
2. Request translation to DeepSeek OpenAI-compatible endpoint.
3. Response & SSE streaming translation.
4. Telemetry extraction for 64-token automatic context caching.
...
class DeepSeekAdapter(LLMProvider):
    """Production adapter for DeepSeek V3 / R1 reasoning models."""
    ...
    async def _resolve_api_key(self, request: ProviderRequest) -> str:
        ...
        raise ProviderAuthenticationError(
            message="No DeepSeek API key configured (neither tenant BYOK nor platform default)",
    ...
    async def complete(
        ...
        async def _exec(c: httpx.AsyncClient) -> ProviderResponse:
            ...
            telemetry = ProviderCacheTelemetry(
                is_cache_eligible=(total_prompt_tokens >= 64),
                cache_hit=cache_hit,
