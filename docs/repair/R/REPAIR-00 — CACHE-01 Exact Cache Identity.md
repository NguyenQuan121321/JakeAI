...
2. OpenAPI Schema Drift CI Failure: In GitHub Actions CI, the step `API Breaking Change & Schema Drift Gate` runs `python -m app.main --export-openapi openapi.json` followed by `git diff --exit-code openapi.json`. While adding `tools` and `response_format` to `GatewayChatRequest`, `backend/openapi.json` was generated dynamically during the CI run and differed from the baseline committed `backend/openapi.json`. Because `backend/openapi.json` had not been regenerated and committed into the branch, `git diff --exit-code openapi.json` detected uncommitted changes and exited with code 1.
3. CI Pull-Request Test Isolation Cause: In GitHub Actions CI, a live Redis service container runs on `localhost:6379`. Multiple async integration tests in `test_cache_identity.py` shared `tenant_id="t1"` with the identical prompt `"Hello"`. Earlier tests (`test_cache_miss_different_model`) stored cache entries into Redis with `tools=None` and `response_format=None`. Subsequent tests (`test_cache_miss_different_tools` and `test_cache_miss_different_response_format`) queried with `tools=None` and `response_format=None` respectively, encountering cross-test Redis cache hits and failing `assert result is None`.
4. Parameter Dict Ingestion Defect: Callers supplying `tools` or `response_format` via legacy `parameters={...}` or `generation_params={...}` dicts were not normalized into the canonical top-level fields, causing identical requests to compute different cache identities depending on parameter passing style.
...
6. Extended `GatewayChatRequest` in `backend/app/services/ai_gateway.py` to support optional `tools: list[dict[str, Any]] | None = Field(default=None)` and `response_format: dict[str, Any] | str | None = Field(default=None)`.
...
10. Regenerated and committed canonical `backend/openapi.json` using the official `python -m app.main --export-openapi openapi.json` export mechanism.

OPENAPI CONTRACT ANALYSIS:
- Endpoint Affected: `POST /api/v1/gateway/chat/completions` (and `POST /v1/chat/completions`)
- Schema Affected: `components/schemas/GatewayChatRequest`
- Properties Added: `tools` (`anyOf: [array of objects, null]`) and `response_format` (`anyOf: [object, string, null]`)
- Required vs. Optional: Both `tools` and `response_format` are explicitly OPTIONAL. The `required` array of `GatewayChatRequest` contains exclusively `["messages"]`.
- Breaking Change Classification: NON-BREAKING ADDITIVE CHANGE. No existing fields were removed, no required fields were added, no types mutated, and no endpoints/response codes were altered.
- Checker Result: `python scripts/check_openapi_breaking_changes.py` reports: "0 breaking changes detected against baseline revision. Status: 100% Backward Compatible."
...
1. Targeted regressions:
   `backend\.venv\Scripts\python.exe -m pytest backend/tests/unit/test_cache_identity.py -k "test_cache_miss_different_tools or test_cache_miss_different_response_format" -v` -> 2 passed in 1.05s.
2. Complete cache identity regression suite:
   `backend\.venv\Scripts\python.exe -m pytest backend/tests/unit/test_cache_identity.py -v` -> 30 passed in 9.11s.
3. Affected API/OpenAPI contract tests:
   `backend\.venv\Scripts\python.exe -m pytest backend/tests/contract/test_api_contract.py -v` -> 5 passed in 0.48s.
4. OpenAPI breaking-change checker:
   `backend\.venv\Scripts\python.exe scripts/check_openapi_breaking_changes.py` -> 0 breaking changes detected (100% Backward Compatible).
5. Static code analysis & linting:
   `backend\.venv\Scripts\python.exe -m ruff check backend/app/optimizer/semantic_cache.py backend/app/services/ai_gateway.py backend/tests/unit/test_cache_identity.py backend/tests/contract/test_api_contract.py` -> All checks passed.
...
9. Relevant subsystem tests:
   `backend\.venv\Scripts\python.exe -m pytest backend/tests/test_semantic_cache.py backend/tests/evals/test_phase03_token_optimization.py -v` -> 17 passed in 3.32s.
10. CI OpenAPI validation sequence:
    `python -m app.main --export-openapi openapi.json` followed by `git diff --exit-code openapi.json` -> Exit code 0 (zero drift).

ACTUAL RESULTS:
- 100% pass rate on all 30 cache identity regression tests and existing contract/cache tests.
...
- [PASS] tools participate in exact cache identity.
- [PASS] response_format participates in exact cache identity.
- [PASS] GET and SET use the same cache identity contract.
- [PASS] test_cache_miss_different_tools passes.
- [PASS] test_cache_miss_different_response_format passes.
- [PASS] complete cache regression suite passes (30/30 passed).
- [PASS] generated openapi.json matches committed openapi.json (`git diff --exit-code openapi.json` returns 0).
- [PASS] OpenAPI breaking-change checker passes (0 breaking changes detected).
- [PASS] no unintended API breaking change exists (additive optional fields only).
